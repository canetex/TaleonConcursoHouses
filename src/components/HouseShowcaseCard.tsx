...
interface HouseShowcaseCardProps {
  house: House
  leaderboard_entry?: LeaderboardEntry | null
  show_vote_link?: boolean
  show_detail_link?: boolean
}

export function HouseShowcaseCard({
  house,
  leaderboard_entry = null,
  show_vote_link = true,
  show_detail_link = false,
}: HouseShowcaseCardProps) {
          ...
          <div className="relative bg-black/40 min-h-[240px] max-h-[70vh] w-full overflow-hidden">
            ...
            {images.length > 1 && (
              <>
                ...
                <span className="absolute bottom-2 left-3 text-[10px] text-amber-200/50 bg-black/50 px-2 py-0.5 rounded">
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-5 text-sm">
            ...
            <div className="p-3 rounded-xl bg-tibia-dark/60">
              ...
              <p className="text-amber-100">{status_labels[house.status]}</p>
            </div>
          </div>
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-3">
            {show_detail_link && (
              <Link
                to={`/house/${house.id}`}
                className="flex-1 text-center py-3 rounded-xl border border-amber-700/40 text-amber-100 font-medium hover:bg-tibia-dark/60 transition-colors"
              >
                Ver página completa
              </Link>
            )}
            {show_vote_link && house.status === 'approved' && (
              <Link
                to="/votar"
                className="flex-1 text-center py-3 rounded-xl bg-tibia-green/80 hover:bg-tibia-green text-amber-50 font-medium transition-colors"
              >
                Votar nesta Casa
              </Link>
            )}
